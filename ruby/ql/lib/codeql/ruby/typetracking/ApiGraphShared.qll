...
signature module ApiGraphSharedSig {
   ...
   * the API graph to include representives for sinks. Note that there is no corresponding case for "source"
...
module ApiGraphShared<ApiGraphSharedSig S> {
  ...
  bindingset[node]
  pragma[inline_late]
  private TypeTrackingNode getALocalSourceStrict(Node node) { result = node.getALocalSource() }
  ...
  private module Cached {
    ...
    predicate epsilonEdge(ApiNode pred, ApiNode succ) {
      ...
      |
        StepSummary::Internal::stepCall(predNode, succNode, summary)
        or
        StepSummary::Internal::stepNoCall(predNode, succNode, summary)
      |
        pred = getForwardNode(predNode, predState) and
        succState = StepSummary::Internal::append(predState, summary) and
    ...
    /**
     * The reflexive, transitive closure of `epsilonEdge`.
     */
    cached
    ...
    ApiNode forwardStartNode(TypeTrackingNode source) {
      result = getForwardNode(source, StepSummary::Internal::noContentTypeTracker(false))
    }
    ...
    ApiNode backwardStartNode(TypeTrackingNode sink) {
      ...
      result = getBackwardNode(sink, StepSummary::Internal::noContentTypeTracker(_))
    }
  ...
  private ApiNode forwardEndNode(TypeTrackingNode localSource) {
    result = getForwardNode(localSource, StepSummary::Internal::noContentTypeTracker(_))
  }
  ...
  private ApiNode backwardEndNode(TypeTrackingNode localSource) {
    result = getBackwardNode(localSource, StepSummary::Internal::noContentTypeTracker(false))
  }

  /** Gets a node reachable from `node` by zero or more epsilon edges, including `node` itself. */
  bindingset[node]
  pragma[inline_late]
  ApiNode getEpsilonSuccessorInline(ApiNode node) { epsilonStar(node, result) }
  ...
  Node getAValueReachableFromSourceInline(ApiNode source) {
    exists(TypeTrackingNode src |
      src = asSourceInline(getEpsilonSuccessorInline(source)) and
  ...
  Node getAValueReachingSinkInline(ApiNode sink) {
    result = asSinkInline(getEpsilonSuccessorInline(sink))
  }
