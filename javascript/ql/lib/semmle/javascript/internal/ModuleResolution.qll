...
private predicate shouldFindRootValue(DataFlow::SourceNode node) {
  ...
  (
    ...
    or
    storeStep(trackExtended(_).getALocalUse(), _, node)
    or
    ...
    exists(DataFlow::SourceNode next | shouldFindRootValue(next) |
      ...
      storeReadStep(node.getALocalUse(), next) or
      extendStep(node, next)
    )
  )
}
...
pragma[nomagic]
DataFlow::SourceNode trackExtended(DataFlow::SourceNode node) {
  extendStep(track(node), result)
  or
  extendStep(trackExtended(node), result)
  or
  valueBigStep(trackExtended(node), result)
  or
  storeReadStep(trackExtended(node).getALocalUse(), result)
}

pragma[nomagic]
private predicate extendStep(DataFlow::SourceNode node1, DataFlow::SourceNode node2) {
  exists(ExtendCall call |
    node1 = call.getASourceOperand().getALocalSource() and
    (
    ...
    or
    node1 = call.getDestinationOperand().getALocalSource() and
    node2 = call
  )
}
...
private predicate deepStore(DataFlow::SourceNode object, string prop, DataFlow::Node value) {
  ...
  or
  storeStep(value, prop, trackExtended(object))
}
...
private predicate deepRead(DataFlow::SourceNode object, string prop, DataFlow::SourceNode value) {
  ...
  or
  readStep(trackExtended(object), prop, value)
}
