...
module UniversalTypeTrackingGen<UniversalTypeTrackingSig S> {
  ...
  private module Cached {
     ...
     */
    cached
    Node trackNode(Node source) {
      result = trackNode(source, any(Summary s | s.getTransformation().isEmpty()))
    }

    /**
     * Gets a node reachable from `source` while undergoing the given `transformation`.
     */
    cached
    Node trackNodeWithTransformation(Node source, Transformation transformation) {
      result = trackNode(source, any(Summary s | s.getTransformation() = transformation)) and
      not transformation.isEmpty()
    }
    ...
    /**
     * Gets a node which flows to `sink` while undergoing the given `transformation`.
     */
    cached
    Node backtrackNodeWithTransformation(Node sink, Transformation transformation) {
      sink = trackNode(result, any(Summary s | s.getTransformation() = transformation)) and
      not transformation.isEmpty()
    }
  }
