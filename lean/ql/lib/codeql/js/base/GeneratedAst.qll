...
module JS {
  ...
  private @file getNodeFile(@js_ast_node node) {
    exists(@location_default loc | js_ast_node_location(node, loc) |
      locations_default(loc, result, _, _, _, _)
    )
  }

  /** Holds if `node` is in the `file` and is part of the overlay base database. */
  private predicate discardableAstNode(@file file, @js_ast_node node) {
  ...
  overlay[discard_entity]
  private predicate discardAstNode(@js_ast_node node) {
    exists(@file file, string path | files(file, path) |
