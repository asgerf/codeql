...
pub fn run(_: Options) -> std::io::Result<()> {
    let database = env::var("CODEQL_EXTRACTOR_LEAN_WIP_DATABASE")
        .expect("CODEQL_EXTRACTOR_LEAN_WIP_DATABASE not set");

    autobuilder::Autobuilder::new("lean", PathBuf::from(database))
